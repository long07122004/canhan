...
<body>
<form action="/ctsp/update?id=${detal.idctsp}" method="post">
    <div class="mb-3">
        ...
        <select id="disabledSelect" class="form-select" name="sanpham">
            <c:forEach items="${sanPham}" var="sanPham">
                <option value="${sanPham.id}"
                        <c:if test="${detal.sanpham.id == sanPham.id}">selected</c:if>
        ...
        <select id="disabledSelectt" class="form-select" name="mausac">
            <c:forEach items="${mauSac}" var="mauSac">
                <option value="${mauSac.id}"
                        <c:if test="${detal.mausac.id == mauSac.id}">selected</c:if>
        ...
        <select id="disabledSelecttt" class="form-select" name="sizesp">
            <c:forEach items="${sizeSp}" var="sizeSp">
                <option value="${sizeSp.id}"
                        <c:if test="${detal.sizesp.id == sizeSp.id}">selected</c:if>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="soluongton" value="${detal.soluongton}" >
    </div>
